...
**Architecture: GitHub Actions-based scheduling.** The daily digest is generated
on GitHub at 6:15am HKT via a GitHub Actions workflow, then committed to the user's
Obsidian vault repository. The user's MacBook only needs to `git pull` when online —
...
- **Qwen** (`PLATFORM=qwen`): The skill runs within Qwen Code. For scheduled
  delivery at 6:15am daily, a GitHub Actions workflow on the user's `obsidian-vault`
  repo handles everything. The user's MacBook only needs to `git pull` when online
  — no local cron dependency on internet connectivity.
...
### Step 7: Set Up GitHub Actions Workflow (6:15am HKT)

The digest is generated by a GitHub Actions workflow on the user's `obsidian-vault`
repository — not by local cron. This ensures the digest is always generated on time,
even when the MacBook is sleeping or offline.

Tell the user: "排程任務已設定。每天早上6:15（香港時間），GitHub Actions 會自動生成 AI Builders 摘要並提交到您的 Obsidian Vault 倉庫。您只需在電腦上線時 git pull 即可。"
...
This workflow runs via GitHub Actions (6:15am HKT daily) or when the user manually invokes it.

### How it works

The GitHub Actions workflow (`.github/workflows/daily-digest.yml`) handles the entire pipeline:
1. Clones the follow-builders-obsidian skill repo
2. Runs `prepare-digest.js` to fetch content feeds
3. Runs `run-digest-ga.js` which calls OpenAI API to remix + translate content
4. Writes the digest to `AI News Daily/YYYY-MM-DD.md` and `YYYY-MM-DD.md`
5. Commits and pushes to the vault repo

The user's MacBook only needs to `git pull` the vault repo when online to receive the digest.

### Manual invocation by the agent

When the user asks the agent to generate a digest on-demand, the agent runs:
...
**For GitHub Actions (scheduled runs):**
The workflow writes the digest directly to `AI News Daily/YYYY-MM-DD.md` and `YYYY-MM-DD.md`
in the vault repo, then commits and pushes. The user receives it via `git pull`.

**For manual agent invocation (on-demand):**
The agent uses `prepare-digest.js` + OpenAI API + local file write:

```bash
cd ${SKILL_DIR}/scripts && node prepare-digest.js 2>/dev/null
# Agent remixes content and writes to vault directly
```

The agent writes the digest to the vault at the appropriate date file.
